...
export const Heatmap = ({ title, filters, periods, items }: HeatmapProps) => {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className="text-xs text-slate-500">기준: {activePeriod}</span>
      </div>
        ...
        {periods.map((period) => (
          <button
            ...
            className={
              ...
              (activePeriod === period ? 'bg-slate-200 text-slate-900' : 'bg-slate-100 text-slate-600 hover:bg-slate-200')
            }
          >
            {period}
          </button>
